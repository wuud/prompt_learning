...
for seed in [2, 144, 145]:
    for shot in [1, 4, 8, 16]:
        ...
        # CL_epoch = 5
        # CL_lr = 1e-5
        # contrastive_learning.SimCSE_train(train_dataset, tokenizer, max_length, model_path,
        #                                   save_path, plm, CL_epoch, CL_lr)

        prompt_template = ManualTemplate(text='{"placeholder":"text_a" }。这是{"mask"}的。',
                                         tokenizer=tokenizer)  # template1
        # prompt_template = ManualTemplate(text='{"placeholder":"text_a" }。这包括{"mask"}。', tokenizer=tokenizer)  # template2 0.6625
        ...
        # data = data_aug.aug_dataset(train_dataset)
        ### 对比学习
        # cl_train_dataloader, cl_model = cl.get_cl_module(data, tokenizer, max_length, plm, model_config, batch_size=4 * batch_size)
        ...
        def train(EPOCH):
            ...
            for epoch in range(EPOCH):  # Longer epochs are needed
                ...
                print('cl tarin loader len: ', len(cl_train_dataloader))
                for step, (data, inputs) in enumerate(zip(cl_train_dataloader, train_dataloader)):
                    # print('step: ', step)
                    ...
                    if data is not None:
                        ...
                        pred = cl_model(input_ids, attention_mask, token_type_ids)
                        loss2 = cl.compute_loss(pred)
                    else:
                        loss2 = 0
                ...
                print("Epoch {}, loss1: {}, loss2: {}, total loss: {}".format(epoch, loss1.item(), loss2.item(),
        ...
        with open(f'./res1/seed{seed}_shot{shot}.txt', 'w') as f:
